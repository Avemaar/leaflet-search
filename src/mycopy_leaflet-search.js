...
L.Control.Search = L.Control.extend({
	...
	options: {
		...
		url1: '',					//url for search by ajax request, ex: "search.php?q={s}"
		url2: '',					//url for search by jsonp request, ex: "nominate"
...
//////end DOM creations
	...
	_recordsFromJsonp: function(text, callAfter) {  //extract searched records from remote jsonp service
		...
		var script = L.DomUtil.create('script','search-jsonp', document.getElementsByTagName('body')[0] ),			
			url = L.Util.template(this.options.url2+'&'+this.options.jsonpParam+'=L.Control.Search.callJsonp', {s: text}); //parsing url
	...
	_recordsFromAjax: function(text, callAfter) {	//Ajax request
		...
		var request = new XMLHttpRequest(),
			url = L.Util.template(this.options.url1, {s: text}), //parsing url
			//rnd = '&_='+Math.floor(Math.random()*10000);
